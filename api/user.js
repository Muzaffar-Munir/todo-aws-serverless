...
const candidateInfo = (id, fullname, email, experience) => {
...
module.exports.updateItem = (event, context, callback) => {
  ...
  submitCandidateP(candidateInfo(event.pathParameters.id, fullname, email, experience))
    .then(res => {
      callback(null, {
        statusCode: 200,
        body: JSON.stringify({
          message: `Sucessfully update candidate with email ${email}`,
          candidateId: res.id
        })
      });
    })
    .catch(err => {
      ...
      callback(null, {
        statusCode: 500,
        body: JSON.stringify({
          message: `Unable to update candidate with email ${email}`
        })
      })
    });
};
